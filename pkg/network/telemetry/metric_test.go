...
func TestMetricOperations(t *testing.T) {
	...
	t.Run("regular (non-synced) metric", func(t *testing.T) {
		Clear()
	...
	t.Run("synchronized metric", func(t *testing.T) {
		Clear()

		m1 := NewMetric("m1", OptSync)
		m1.Add(int64(5))

		// we haven't called Sync() yet, so the returned value is still zero
		assert.Equal(int64(0), m1.Get())

		m1.Add(int64(5))
		m1.Sync()
		// now we should get back 5+5 since we called Sync()
		assert.Equal(int64(10), m1.Get())

		// calling sync is idempotent
		m1.Sync()
		assert.Equal(int64(10), m1.Get())
	})
}

func TestSeparateTagsAndOpts(t *testing.T) {
	assert := assert.New(t)

	t.Run("only tags", func(t *testing.T) {
		tags, opts := separateTagsAndOptions([]string{"tag:a", "tag:c", "tag:b"})
		assert.Equal([]string{"tag:a", "tag:b", "tag:c"}, tags)
		assert.Len(opts, 0)
	})

	t.Run("only opts", func(t *testing.T) {
		tags, opts := separateTagsAndOptions([]string{"_opt3", "_opt2", "_opt1"})
		assert.Equal([]string{"_opt1", "_opt2", "_opt3"}, opts)
		assert.Len(tags, 0)
	})

	t.Run("tags and opts", func(t *testing.T) {
		tags, opts := separateTagsAndOptions(
