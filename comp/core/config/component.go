...
// component temporarily wraps pkg/util/config.
//
// The component assumes that pkg/util/config has been initialized already and
// simply provides access to that configuration.  When no code still uses
// pkg/util/config, that will change.
//
// The mock component does nothing at startup, beginning with an empty config.
// It also overwrites the pkg/util/config.Datadog for the duration of the test.
package config
